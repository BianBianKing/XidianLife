        ...
        <div class="panel-body">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
	                    <c:forEach items="${collegeList }" var="node" varStatus="s">
				        	<tr>
			          	    <td><c:out value="${s.index+1 }"></c:out></td>
			          	    
					        <td><c:out value="${node.college_name }"></c:out></td>  
					        <td>
					        	<a href="editCollege.htm?college_id=${node.id }" class="btn btn-info btn-xs">
                                    <i class="fa fa-pencil"></i>修改
                                </a>
                                <a href="delCollege.action?college_id=${node.id }" class="btn btn-danger btn-xs">
                                    <i class="fa fa-trash-o"></i>删除
                                </a>
                                <a href="classtableList.htm?college_id=${node.id }" class="btn btn-s-md btn-primary btn-rounded btn-xs">查看学院班级</a>
                            </td>  
	                        </tr>
				        </c:forEach>
                    </tbody>
                </table>
                <a href="addCollege.htm" class="btn btn-s-md btn-dark pull-right" style="margin-top:23px">添加学院</a> 
            </div>
            <!-- /.table-responsive -->
        </div>
        <!-- /.panel-body -->
