        ...
        <div class="panel-body">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    ...
                    <tbody>
	                    <c:forEach items="${competitionList }" var="node" varStatus="s">
				        	<tr>
			          	    <td><c:out value="${s.index+1 }"></c:out></td>
			          	    
					        <td><c:out value="${node.title }"></c:out></td>  
					        <td><c:out value="${node.date }"></c:out></td>  
					        <td><a href="detailCompetition.htm?id=${node.id }" class="btn btn-s-md btn-primary btn-rounded btn-xs">进入</a></td>
					        <td>
					        	<a href="modifCompetition.htm?id=${node.id }" class="btn btn-info btn-xs">
                                    <i class="fa fa-pencil"></i>修改
                                </a>
                                <a href="deleteCompetition.action?id=${node.id }" class="btn btn-danger btn-xs">
                ...
                <a href="addCompetition.htm" class="btn btn-s-md btn-dark pull-right" style="margin-top:23px">添加竞赛公告</a> 
            </div>
            <!-- /.table-responsive -->
        </div>
        <!-- /.panel-body -->
